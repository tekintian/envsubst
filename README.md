...
- **🆕 条件替换**：`${VAR:+value}` 真值判断（on/yes/1/true），智能控制输出
- **🆕 嵌套变量**：完全支持 `${A:+${B:-default}}` 多层嵌套解析
- **🆕 多行块支持**：使用 `\n` 实现多行条件配置
...
```bash
# 基本用法：变量为真值时输出
echo '${ENABLED:+feature is on}' | ENABLED=on envsubst
# 输出: feature is on

echo '${ENABLED:+feature is on}' | ENABLED=off envsubst
# 输出: (空)

# 支持的真值：on/yes/y/true/t/1/enabled/enable（不区分大小写）
echo '${VAR:+yes}' | VAR=YES envsubst     # 输出: yes
echo '${VAR:+yes}' | VAR=1 envsubst       # 输出: yes
echo '${VAR:+yes}' | VAR=true envsubst    # 输出: yes

# 假值：off/no/n/false/0/disabled 等
echo '${VAR:+yes}' | VAR=0 envsubst       # 输出: (空)
echo '${VAR:+yes}' | VAR=no envsubst      # 输出: (空)
```

### 5. 🆕 嵌套变量支持
```bash
# 嵌套默认值
echo '${A:+${B:-default}}' | A=on B=value envsubst
# 输出: value

echo '${A:+${B:-default}}' | A=on envsubst
# 输出: default

echo '${A:+${B:-default}}' | A=off envsubst
# 输出: (空)

# 多层嵌套
echo '${A:+prefix=${B:+inner=${C:-fallback}}}' | A=on B=yes C=test envsubst
# 输出: prefix=inner=test
```
...
# 启用 binlog
MYSQL_ENABLE_BINLOG=on MYSQL_LOG_BIN=custom-bin envsubst < config.tpl
# 输出:
# log-bin=custom-bin
# binlog_format=ROW
# expire_logs_days=7
...
# 禁用 binlog
MYSQL_ENABLE_BINLOG=off envsubst < config.tpl
# 输出: (空)
```

### 7. 启用全模式替换（兼容原生 envsubst）
```bash
...
# 使用白名单文件
envsubst --whitelist-file whitelist.txt < config.tpl > config.conf
```
